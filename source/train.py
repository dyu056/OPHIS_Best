...
def observe_attention_distribution(prefix, q, k, window_size, sample_tokens=64):
    ...
    try:
        track_grad = ATTENTION_ENTROPY_REG_CAPTURE_ACTIVE or any(
            REG_CAPTURE.wants(f"{prefix}.{suffix}")
            for suffix in (
                "attn_entropy",
                "attn_entropy_norm",
                "attn_max_prob",
        ...
        with nullcontext() if track_grad else torch.no_grad():
            ...
            for suffix, value in values.items():
                ...
                REG_CAPTURE.add(f"{prefix}.{suffix}", value)
    except Exception:
        return
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size, bigram_ve=None, trigram_ve=None):
        ...
        if OBSERVE_LAYER_PROBES or REG_CAPTURE.wants_layer(self.layer_idx) or ATTENTION_ENTROPY_REG_CAPTURE_ACTIVE:
            prefix = f"layer_{self.layer_idx}"
            if OBSERVE_LAYER_PROBES:
                OBS.add_norms(f"{prefix}.q", q)
                OBS.add_norms(f"{prefix}.k", k)
                OBS.add_norms(f"{prefix}.v", v)
            REG_CAPTURE.add_norms(f"{prefix}.q", q)
            REG_CAPTURE.add_norms(f"{prefix}.k", k)
            REG_CAPTURE.add_norms(f"{prefix}.v", v)
            ...
            if (
                ...
                or REG_CAPTURE.observable.startswith(attention_probe_prefixes)
        ...
        if OBSERVE_LAYER_PROBES or REG_CAPTURE.wants(f"layer_{self.layer_idx}.head_gate_mean"):
            REG_CAPTURE.add(f"layer_{self.layer_idx}.head_gate_mean", head_gates)
        if OBSERVE_LAYER_PROBES:
        ...
        REG_CAPTURE.add_norms(f"layer_{self.layer_idx}.attn_out", y)
        y = self.c_proj(y)
        return y
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        REG_CAPTURE.add_norms(f"layer_{self.layer_idx}.mlp_pre", h_pre)
        REG_CAPTURE.add_norms(f"layer_{self.layer_idx}.mlp_act", h_act)
        REG_CAPTURE.add_norms(f"layer_{self.layer_idx}.mlp_out", h)
        return h
...
NGRAM_VE_BETAS = (0.5, 0.9995) # RMSProp only uses beta2=0.999; higher beta2 preserves gradient history for sparse tables
NGRAM_VE_LR_SCALE = 1.2 # RMSProp with full LR (no reduction)
...
if OBSERVE_LAYER_PROBES or OBSERVE_HESSIAN_EIGENVALUES or REG_CAPTURE.enabled or ATTN_ENTROPY_REG:
...
NGRAM_VE_BETA2_WARMDOWN = 0.99995 # STRONGER delayed VE beta2 ramp (0.999->0.9999 last 30% warmdown)
